...
- 本地上传：本地能运行的测试，上传测试的JMX，CSV，JAR等依赖，平台均可直接处理和执行，日志，报告
  ```
  - 新建用例：会在Master节点创建用例命名空间
  - 上传JMX：本地调试好的JMX（比如已经关联了本地的CSV和JAR）上传到对应用例目录下
  - 上传CSV：有一个限制，本地调试的时候，CSV Data Set Config的Name需要设置为上传的CSV文件名带后缀，以便平台修改CSV模块
  - 上传JAR：没啥限制，平台会修改JMX脚本的ClassPath模块
  ```
...
容器化部署通过docker-compose方式一键部署，如果在线拉镜像不畅，网盘里下最新版本离线容器镜像（包括x86和arm64）
...
>- sh init.sh amd64|arm64 （参数：x86传amd64，arm传arm64；init.sh可自定义平台运行的${BASE_DIR}目录）
>- cd ./mysterious
...
### ⚙️ Slave节点：压测引擎部署（分布式压测，Slave节点作为压力机，启动Jmeter-Server服务）
